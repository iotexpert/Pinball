...
static Note *`$INSTANCE_NAME`_Songs[`$INSTANCE_NAME`_MAX_SONGS];
...
static void `$INSTANCE_NAME`_NextNote()
{
    ...
    int period = `$INSTANCE_NAME`_frequencyToPeriod(`$INSTANCE_NAME`_Songs[`$INSTANCE_NAME`_currentSong][`$INSTANCE_NAME`_nextNote].frequency);
...
{
    `$INSTANCE_NAME`_AddSong(0,`$INSTANCE_NAME`_twinkleLittleStar);
#ifdef `$INSTANCE_NAME`_TWOCHANNELS
...
`$INSTANCE_NAME`_ErrorCodes `$INSTANCE_NAME`_PlaySong(int number,int bpm)
{
    ...
    if(`$INSTANCE_NAME`_Songs[number] == (Note *)0)
        return NO_SONG;
    
    `$INSTANCE_NAME`_currentSong = number;
    rval = `$INSTANCE_NAME`_SetBPM(bpm);
    `$INSTANCE_NAME`_nextNote = 1; // wrap around to the start
    `$INSTANCE_NAME`_NextNote();
    return rval;
}
/*****************************************************************************\
